...
def createbase():
    ...
    c = conn.cursor()
    c.execute("CREATE TABLE config (id integer UNIQUE PRIMARY KEY,"
              " indice VARCHAR(30), value INTEGER)")
...
def new_value(indice, value):
    ...
    c = conn.cursor()
    valor = str(value)
    c.execute("INSERT INTO config VALUES(NULL, '" +
              indice + "', " + str(value) + ")")
    conn.commit()
    c.close()


def try_value(indice, value):
    """tries if exist indice on database.
    If exists: nothing, else creates a indice with indicated value"""
    try:
        get_value(indice)
    except:
